...
let baseURI = '/apps/' + config.HEROKU.APP_NAME;
   ...
   var l_dsc = ''
    ...
    if (config.LANG == 'EN') {
        l_dsc = 'autobio it set live clock.-bot owner command'
        alr_on = 'autobio is already open!'
        alr_off = 'autobio is currently closed!'
        BGM_on = 'autobio option turned on!'
        BGM_off = 'autobio option turned off'
    }
    if (config.LANG == 'SI') {
        l_dsc = 'autobio system  ක්‍රියාත්මක කිරීම.'
        alr_on = 'කලින්ම ක්‍රියාත්මකයි!'
        alr_off = 'කලින්ම අක්‍රියයි!'
        BGM_on = '*autobio ක්‍රියාත්මයි.*'
        BGM_off = '*autobio අක්‍රියයි.*'
    }
    if (config.LANG == 'HI') {
    ...
    Asena.addCommand({pattern: 'autobio ?(.*)', fromMe: true, desc: l_dsc, usage: '.autobio on / off' }, (async (message, match) => {
        if (match[1] == 'off') {

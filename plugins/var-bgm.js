...
let baseURI = '/apps/' + config.HEROKU.APP_NAME;
   ...
   var l_dsc = ''
    ...
    if (config.LANG == 'EN') {
        l_dsc = 'bgm option.-bot owner command'
        alr_on = 'bgm option is already open!'
        alr_off = 'bgm option currently closed!'
        BGM_on = 'bgm option turned on!'
        BGM_off = 'bgm option turned off'
    }
    if (config.LANG == 'SI') {
        l_dsc = 'bgm option  ක්‍රියාත්මක කිරීම.'
        alr_on = 'කලින්ම ක්‍රියාත්මකයි!'
        alr_off = 'කලින්ම අක්‍රියයි!'
        BGM_on = '*bgm option ක්‍රියාත්මයි.*'
        BGM_off = '*bgm option අක්‍රියයි.*'
    }
    if (config.LANG == 'HI') {

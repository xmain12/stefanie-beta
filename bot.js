    ...
    setInterval(async () => { 
        if (biography_var == 'true') {
           if (conn.user.jid.startsWith('994')) { // Azerbayjan
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time +'\n\n ' + config.BIO +' \nᴾᵒʷᵉʳᵉᵈ ᴮʸ*© 2021 | STEFANIE V 8.5.2 beta👸*'
...
// =====================DEBUG=================
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('Queen')}${chalk.blue.bold('Stefanie')}
...
${chalk.blue.italic('👩‍🦰 Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('🛡️Stefanie working!')
        );
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'SI' || config.LANG == 'AZ') {
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*👸QUEEN STEFANIE As Public! 👩‍🦰*\n\n_මෙය ඔබගේ LOG අංකයයි..මෙහි විධාන භාවිතයෙන් වළකින්න._\n_ඔබට පුලුවන් වෙනත් ඕනෑම කතා බහක විධාන භාවිතා කිරීමට.. :)_\n\n*ඔබේ Bot Public ආකාරයට ක්‍රියා කරයි..එය වෙනස් කිරීමට* _.setvar WORK_TYPE:private_ *විධානය භාවිතා කරන්න.*\n\n*Bot ක්‍රියාත්මක වන්නෙ කෙසේද හා විධාන ලැයිස්තු ලබා ගැනීමට⚜ .help විධානය භාවිතා කරන්න*\n\nSupport Group : https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4\n*🧚‍♂QUEEN STEFANIE භාවිතා කිරීම සම්බන්ධයෙන් ස්තූතියි 💌*', MessageType.text);
                }
                await git.fetch();
            ...
            if (config.LANG == 'SI' || config.LANG == 'AZ') { 
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*🧚‍♂QUEEN Stefanie As private! 👩‍🦰*\n\n_මෙය ඔබගේ LOG අංකයයි..මෙහි විධාන භාවිතයෙන් වළකින්න._\n_ඔබට පුලුවන් වෙනත් ඕනෑම කතා බහක විධාන භාවිතා කිරීමට.. :)_\n\n*ඔබේ Bot Private ආකාරයට ක්‍රියා කරයි..එය වෙනස් කිරීමට* _.setvar WORK_TYPE:public_ *විධානය භාවිතා කරන්න.*\n\n*Bot ක්‍රියාත්මක වන්නෙ කෙසේද හා විධාන ලැයිස්තු ලබා ගැනීමට⚜ . help විධානය භාවිතා කරන්න*\n\nSupport Group : _https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4_\n*🧚‍♂QUEEN STEFANIE භාවිතා කිරීම සම්බන්ධයෙන් ස්තූතියි 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*🧚‍♂QUEEN stefanie Working As private!👩‍🦰\n\nPlease do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.basichelp* _to get your full Help list and Basic Commands._\n\n_Your bot in private  Mode. To change, use_ ```.setvar WORK_TYPE:public``` _command._\n\n*Thanks for using 🧚‍♂QUEEN STEFANIE💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
...
*/
                        try {
                            ...
                            if (config.LANG == 'SI' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- බොට් වාර්තාව [🛡️Queen-STEFANIE] --*' + 
                                    '\n*👩‍🦰Queen-Stefanie නිසි ලෙස ක්‍රියා කරයි!*'+
                                    ...
                                    '\n_සහය සමූහ: https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4\n\n' +
                                    '*🚫ප්‍රධාන දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false});

                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚜ 🙇දෝශ විශ්ලේෂනය [👩‍🦰Queen-Stefanie] ⚜*' + 
                                        '\n========== ``` 🧚‍♂දෝශ විශ්ලේෂනය!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ◁දෝශ වාර්තාව▷ [👩‍🦰Queen-Stefanie] ⚕️*' + 
                                        '\n========== ``` 🧚‍♂දෝශ විශ්ලේෂනය!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 කණගාටුයි AUTO ERROR පද්ධතියට හදුනාගත නොහැකියි! 🙇🏻*' +
                                        '\n_ඔබට පුලුවන් වැඩිදුර උදව් සදහා සහය සමූහයට ලිවීමට..🛡️ https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4_'
                                        , MessageType.text
                                    );
                                }
                            }
                            else {
                                await conn.sendMessage(conn.user.jid, '*-- Bot Report [👩‍🦰Queen-Stefanie] --*' + 
                                    '\n*👩‍🦰Queen-Stefanie Working Perfectly!*'+
                                    '\n_This is Your LOG Number Dont try Command here.!_' +
                                    '\n_Also you can Join our Support group.._' +
                                    '\n_Support groups_: https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4_' +
                                    '\n_(saved Messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [👩‍🦰Queen-Stefanie] ⚕️*' + 
                                        '\n========== ```🧚‍♂ERROR ANALYSIS!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🧚‍♂Sorry, I Couldnt Read This Error!🙇🏻*' +
                                        '\n_You can write to our support groups for more help...🛡️ https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4_'
                                        , MessageType.text
                                    );
                               }
                          }
